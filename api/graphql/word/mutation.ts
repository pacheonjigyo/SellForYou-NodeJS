            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    ...
                    if (word2) { // 변경하고자 하는 동일 단어 있는경우
                        ...
                        // if (word2.id !== word.id) { // 변경하고자하는 단어 ID가 이 단어가 아닐 경우
                        //     if (word2.replaceWord === null) {
                        //         await ctx.prisma.wordTable.delete({ where: { id: word.id } });
                        //         return true;
                        //     }
                        //     else {
                        //         await ctx.prisma.wordTable.delete({ where: { id: word2.id } });
                        //         await ctx.prisma.wordTable.update({ where: { id: word.id }, data: { replaceWord } });
                        //         return true;
                        //     }
                        // }
                    ...
                    // await ctx.prisma.wordTable.update({
                    //     where: { id: args.wordId },
                    //     data: { findWord: args.findWord, replaceWord: args.replaceWord },
                    // });
                    return true;
                } catch (e) {
                    return throwError(e, ctx);
                ...
                try {
                    ...
                    try {
                        worksheet.eachRow((r, i) => {
                            ...
                            let replaceWord = r.getCell(2).text.trim();
                    ...
                    // await ctx.prisma.wordTable.createMany({
                    //     skipDuplicates: true,
                    //     data: data.map(v => ({ userId: ctx.token!.userId!, findWord: v[0], replaceWord: v[1] })),
                    // })
                    return true;
                } catch (e) {
                    return throwError(e, ctx);
