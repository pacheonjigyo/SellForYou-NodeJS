...
export const mutation_taobao_product = extendType({
    ...
    definition(t) {
        ...
        t.field("getTaobaoItemUsingExtensionByUser", {
            ...
            resolve: async (src, args, ctx, info) => {
                try {
                    // console.log("ExternalData", args.data);
                    // return "개발중입니다."
                    ...
                    if (data.onebound?.item && (data.sellforyou?.data?.length > 0)) {
                        ...
                        // translatedData.video = taobaoData.video as unknown as string | null;
                        // console.log(translatedData.video, taobaoData.video)

                        if (translatedData.taobaoNumIid !== taobaoData.num_iid) return throwError(errors.etc("원문/번역데이터 고유값이 다릅니다."), ctx);
                        ...
                        const checkUserId = await ctx.prisma.product.findMany({
                            where: { userId: ctx.token?.userId ?? null, taobaoProduct: { taobaoNumIid: taobaoData.num_iid } },
                            select: { taobaoProduct: { select: { taobaoNumIid: true } } }
                        });

                        if (checkUserId.length > 0) {
                            if (ctx.token?.userId) {
                                publishUserLogData(ctx, { type: "getTaobaoItem", title: `${taobaoData.num_iid} 상품은 등록된 상품입니다.` });
                            }
                            return `${taobaoData.num_iid} 상품은 등록된 상품입니다.`;
                        }
                        ...
                        try {
                            const updatedProduct = await ctx.prisma.taobaoProduct.upsert({
                                ...
                                update: {
                                    ...
                                    taobaoCategoryId: item.rootCatId,
                                    name: item.title,
                                    videoUrl: translatedData.video !== null ? translatedData.video : undefined,
                                }
                            })
                        ...
                        const products = await saveTaobaoItemToUser(ctx.prisma, taobaoProducts, userId, info, null, null, ctx.token?.adminId ?? undefined)
                        const resultProducts = products.filter((v): v is Product => v !== null);
                        console.log(`확장 프로그램 : 상품 ${resultProducts.length}개 추가완료 :${resultProducts.map(v => "SFY_" + v.id.toString(36))}`);
                        if (userId) {
                            publishUserLogData(ctx, { type: "getTaobaoItem", title: `상품 ${products.length} 개 중 ${resultProducts.length}개 추가 완료되었습니다.\n상품 ID : ${resultProducts.map(v => "SFY_" + v.productCode).join(",")}` });
                        }

                        return `상품 ${products.length} 개 중 ${resultProducts.length}개 추가 완료되었습니다(상품 ID : ${resultProducts.map(v => v.productCode).join(",")}).`
