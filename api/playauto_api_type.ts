...
export const shopDataUrlInfo: { [key: string]: (data: IShopDataUrlInfo) => string } = {
    ...
    "A006": (data) => `G마켓`,
    "A523": (data) => `G마켓2.0`,
    "A077": (data) => `https://smartstore.naver.com/${data.storeName ?? ""}/products/${data.id}`,
    "A027": (data) => `인터파크 - 오픈마켓`,
    "A112": (data) => `11번가`,
    "A113": (data) => `11번가(단일상품)`,
    "B955": (data) => `마이소`,
    "B118": (data) => `메이크샵`,
    "B378": (data) => `https://www.coupang.com/vp/products/${data.id}${data.vendorId ? `?vendorItemId=${data.vendorId}` : ""}`,
    "B956": (data) => `티몬`,
    "B430": (data) => `위메프`,
    "B719": (data) => `위메프2.0`,
    "C003": (data) => `Qoo10(sg)`,
